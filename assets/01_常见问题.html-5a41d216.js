import{_ as n}from"./plugin-vue_export-helper-c27b6911.js";import{r as s,o as t,c as l,a,b as r,d,f as e}from"./app-e7c8d9dc.js";const c={},h=e(`<h2 id="arraylist-和-linkedlist-的区别" tabindex="-1"><a class="header-anchor" href="#arraylist-和-linkedlist-的区别" aria-hidden="true">#</a> ArrayList 和 LinkedList 的区别</h2><ul><li><p><strong>是否保证线程安全</strong>：二者都是不同步的，都不保证线程安全</p></li><li><p><strong>底层数据结构</strong>：ArrayList 底层是 Object[]数组；LinkedList 底层用的是双向链表</p></li><li><p><strong>插入和删除是否受元素位置的影响</strong>：</p><ul><li>ArrayList 采用数组存储，所以插入和删除元素受元素位置的影响</li></ul><p>比如：执行 add(E e)方法时，会默认将指定的元素追加到此列表的末尾，这种时间复杂度就是 O(1)，但是如果要在指定位置增加元素的话，时间复杂度就是 O(n-i)，因为其余位置的元素都需要向后移动一位</p><ul><li>LinkedList 采用链表存储，所以对于 add(E e)方法来说，复杂度也近似 O(1)，如果是要在指定位置插入元素的话，时间复杂度就近似 O(n)，因为需要先移动到指定位置</li></ul></li><li><p><strong>是否支持快速随机访问</strong>：ArrayList 支持。快速随机访问就是通过元素的序号快速获取元素</p></li><li><p><strong>内存空间占用</strong>：ArrayList 的空间浪费主要体现在 list 的结尾会预留一定的容量空间，而 LinkedList 的空间话费则体现在它的每一个元素都需要消耗比 ArrayList 更多的空间（因为要存放直接后继、直接前驱以及数据）</p></li></ul><h3 id="randomaccess接口" tabindex="-1"><a class="header-anchor" href="#randomaccess接口" aria-hidden="true">#</a> RandomAccess接口</h3><p>ArrayList 实现了此接口，RandomAccess 接口只是一个标识，标识这个类具有随机访问的功能</p><p>在 <code>binarySearch()</code> 方法中，他会判断传入的 list 是否实现 RandomAccess，如果是则调用 <code>indexedBinarySearch()</code> 方法，否则调用 <code>iteratorBinarySearch()</code></p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>public static &lt;T&gt;
